  ...
  const formatContent = (content: string) => {
    return content
      ...
      .map((block, i) => {
        ...
        if (block.startsWith("### ")) {
          return <h3 key={i} className="text-lg font-semibold text-foreground mt-6 mb-2">{block.replace("### ", "")}</h3>;
        }
        ...
        if (block.match(/^\d\./)) {
          const lines = block.split("\n").filter(Boolean);
          return (
            <ol key={i} className="space-y-1.5 my-4 list-decimal list-inside">
              {lines.map((line, j) => (
                <li key={j} className="text-muted-foreground leading-relaxed">{line.replace(/^\d+\.\s*/, "")}</li>
              ))}
            </ol>
          );
        }
