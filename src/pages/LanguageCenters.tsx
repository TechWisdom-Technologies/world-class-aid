...
const CITIES = ["All", "Kuala Lumpur", "Subang Jaya", "Cyberjaya", "Penang"];
...
export default function LanguageCentersPage() {
  ...
  const filtered = useMemo(() => {
    return languageCenters.filter((lc) => {
      ...
      <div className="container mx-auto px-4 py-10">
        <div className="flex flex-col lg:flex-row gap-8" ref={listRef}>
          ...
          <div className="flex-1">
            ...
            {paged.length === 0 ? (
              <div className="text-center py-20 text-muted-foreground">No language programs match your filters.</div>
            ) : (
                    ...
                    <CardContent className="p-5 flex flex-col sm:flex-row sm:items-center gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex flex-wrap items-center gap-2 mt-2">
                          ...
                          <span className="text-xs text-muted-foreground flex items-center gap-1"><DollarSign className="h-3 w-3" /> MYR {lc.tuitionFee.toLocaleString()}</span>
                        </div>
                      </div>
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-center gap-3 mt-8">
                <Button variant="outline" size="sm" disabled={currentPage === 1} onClick={() => { setCurrentPage(currentPage - 1); listRef.current?.scrollIntoView({ behavior: "smooth" }); }}>
                ...
                <Button variant="outline" size="sm" disabled={currentPage === totalPages} onClick={() => { setCurrentPage(currentPage + 1); listRef.current?.scrollIntoView({ behavior: "smooth" }); }}>
