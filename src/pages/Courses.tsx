...
export default function Courses() {
  ...
  const filtered = useMemo(() => {
    return courses.filter((c) => {
      ...
      if (universityId !== "all" && c.university_id !== Number(universityId)) return false;
      return true;
    });
  }, [search, degreeLevel, universityId]);
    ...
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <div className="bg-primary text-primary-foreground py-14">
        <div className="container mx-auto px-4 text-center">
          ...
          <p className="text-primary-foreground/70 max-w-xl mx-auto">Search through {courses.length}+ programs across all partner universities.</p>
        </div>
      </div>
          ...
          <aside className="lg:w-1/4 shrink-0 space-y-6">
            <Card>
              <CardContent className="p-5 space-y-6">
                ...
                <Button variant="outline" className="w-full" onClick={() => { setSearch(""); setDegreeLevel("All"); setUniversityId("all"); }}>
                  Clear Filters
                </Button>
              </CardContent>
            </Card>
                    ...
                    <Card key={c.id} className="hover:shadow-md transition-shadow duration-200">
                      <CardContent className="p-5 flex flex-col sm:flex-row sm:items-center gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-sm text-muted-foreground mt-0.5">{uni?.name}</p>
                          <div className="flex flex-wrap items-center gap-2 mt-2">
                            ...
                            <span className="text-xs text-muted-foreground flex items-center gap-1">
                              <DollarSign className="h-3 w-3" /> USD {c.tuition_fee.toLocaleString()}/yr
                            </span>
                          </div>
                        </div>
